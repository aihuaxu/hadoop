...
public class LeafQueue extends AbstractCSQueue {
  ...
  public CSAssignment assignContainers(Resource clusterResource,
    ...
    public void collectSchedulerApplications(
        ...
        // For optimistic queue, don't return any applications
        if (!getQueueName().toLowerCase().endsWith(YarnConfiguration.LOW_PRIORITY_OPTIMISTIC_QUEUE_SUFFIX)
                && scheduler instanceof CapacityScheduler) {

            // Check and add applications running in optimistic queue corresponding to this queue
            String parentQueueName = getParent().getQueueName().toLowerCase();
            String optimisticQueue = parentQueueName + YarnConfiguration.LOW_PRIORITY_OPTIMISTIC_QUEUE_SUFFIX;

            CSQueue queue = ((CapacityScheduler) scheduler).getQueue(optimisticQueue);
            if (queue != null) {
                Collection<FiCaSchedulerApp> pendingApps = ((LeafQueue) queue).getPendingApplications();
                Collection<FiCaSchedulerApp> activeApps = ((LeafQueue) queue).getApplications();

                Set<FiCaSchedulerApp> allApps = new HashSet<>();
                allApps.addAll(activeApps);
                allApps.addAll(pendingApps);

                for (FiCaSchedulerApp app : allApps) {
                    RMApp rmApp = scheduler.getRMContext().getRMApps().get(app.getApplicationId());

                    if (rmApp != null) {
                        Set<String> appTags = rmApp.getApplicationTags();
                        ...
                        // If app tag contains low priority tag
                        if (caseIgnoreAppTags.contains(YarnConfiguration.LOW_PRIORITY_APP_TAG)) {
                            ...
                            for (String appTag : caseIgnoreAppTags) {
                                if (appTag.startsWith(lowPriorityOriginalQueuePrefix)) {
                                    ...
                                    // if mapped queue is equal to this queue, add the app
                                    if (mappedQueue.equals(getQueueName().toLowerCase())) {
