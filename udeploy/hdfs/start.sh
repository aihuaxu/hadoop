...
function start_router() {
  # Observer router requires -observer flag to start.
  [[ "${container_type}" == "obrouter" ]] && observer_opt="-observer" || observer_opt=""
  source "${HADOOP_ENV}"; exec "${HDFS_START_SCRIPT}" dfsrouter ${observer_opt} 2>&1
}
