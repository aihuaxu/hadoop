...
public class TestStateAlignmentContext {
  ...
  public void testClientSendsState() throws Exception {
    ...
    }).when(spiedAlignContext).updateRequestState(
        Mockito.<RpcHeaderProtos.RpcRequestHeaderProto.Builder>any());
    ...
    assertThat(collectedHeaders.get(0).getStateId(),
        is(not(collectedHeaders.get(collectedHeaders.size() - 1).getStateId())));
