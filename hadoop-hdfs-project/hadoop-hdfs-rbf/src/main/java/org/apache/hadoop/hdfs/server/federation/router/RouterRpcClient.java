...
public class RouterRpcClient {
  ...
  /**
   * When observers are configured for ObserverRouter, we should shuffle Observers
   * since otherwise the requests would go to the same observer until the next
   * NN heartbeats update the order of the observer list.
   */
  @VisibleForTesting
  List<? extends FederationNamenodeContext> shuffleNameNodes(
          List<? extends FederationNamenodeContext> namenodes) {
    if (isObserverRouter &&
            namenodes.get(0).getState() != FederationNamenodeServiceState.ACTIVE) {
      namenodes = new ArrayList<>(namenodes);
  ...
  private Object invokeMethod(
    ...
    namenodes = shuffleNameNodes(namenodes);
