 ...
 * handle readonly operations. It also doesn't participate in either
 * checkpointing or failover.
 */
...
public class ObserverState extends StandbyState {
  ...
  public void checkOperation(HAContext context,
      NameNode.OperationCategory opCategory)
      throws StandbyException {
    if (opCategory == NameNode.OperationCategory.READ &&
        context.observerTooStale()) {
      throw new StandbyException("Observer too stale");
    }
    if (opCategory == NameNode.OperationCategory.UNCHECKED ||
        opCategory == NameNode.OperationCategory.READ) {
      return;
    }
    String faq = ". Visit https://s.apache.org/sbnn-error";
    String msg = "Operation category " + opCategory +
        " is not supported in state " + context.getState() + faq;
