...
public class TestRouterConnectionOverload {
  ...
  private void setupCluster(Configuration conf) throws Exception {
    ...
    NameNodeRpcServer rpcServer = (NameNodeRpcServer) (cluster.getCluster().getNameNode(0).getRpcServer());
    RpcMetrics metrics =
            rpcServer.getClientRpcServer().getRpcMetrics();
    while (metrics.numOpenConnections() != 0) {
      Thread.sleep(1000);
    }
  }
   ...
   */
  private void makeRouterCall(int numOps, int numThreads) throws Exception {
    UserGroupInformation ugi = UserGroupInformation.createUserForTesting(DEFAULT_USER, new String[]{"group1"});
    makeRouterCall(new UserGroupInformation[] {ugi}, numOps, numThreads);
  }

  private void makeRouterCall(UserGroupInformation[] users, int numOps, int numThreads)
      throws Exception {
    RouterContext routerContext = cluster.getRouters().get(0);
    URI address = routerContext.getFileSystemURI();
    Configuration conf = new HdfsConfiguration();
    makeRouterCall(users, address, conf, numOps, numThreads);
  }
