...
public class Balancer {
  ...
  Balancer(NameNodeConnector theblockpool, BalancerParameters p,
    ...
    this.dispatcher =
        new Dispatcher(theblockpool, p.getIncludedNodes(),
            ...
            getBlocksMinBlockSize, blockMoveTimeout, maxNoMoveInterval, maxIterationTime, conf);
  ...
  private void sortOverUtilized(final Map<Source, Double> overUtilizedPercentage) {
    ...
    Collections.sort(list, new Comparator<Source>() {
      @Override
      public int compare(Source o1, Source o2) {
        return Double.compare(overUtilizedPercentage.get(o2),
            overUtilizedPercentage.get(o1));
      }
    });
  }
  ...
  static int run(Collection<URI> namenodes, final BalancerParameters p,
    ...
    while (serviceRunning) {
      try {
        ...
        LOG.warn(
            "Encounter exception while do balance work. Already tried " + exceptionsSinceLastBalance + " times", e);
      }

      // sleep for next round, will retry for next round when it's interrupted
      LOG.info("Finished one round, will wait for " + time2Str(scheduleInterval) + " for next round");
