...
public class TestDataNodeVolumeMetrics {
  ...
  public void testVolumeMetrics() throws Exception {
    ...
    try {
      ...
      DFSTestUtil.getAllBlocks(fs.open(fileName));
  ...
  private void verifyDataNodeVolumeMetrics(final FileSystem fs,
    ...
    LOG.info("TotalBlocksReadFromDisk : " +
        metrics.getBlocksReadFromDisk());
    LOG.info("TotalBytesReadFromDisk : " +
        metrics.getBytesReadFromDisk());
    LOG.info("TotalBytesWrittenToDisk : " +
        metrics.getBytesWrittenToDisk());
  }
}
